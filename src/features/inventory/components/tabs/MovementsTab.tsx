...
export function MovementsTab({
  ...
  const paginatedMovementGroups = useMemo(() => {
    return groupedMovementRows.slice(
      (safeMovementPage - 1) * movementDaysPerPage,
      safeMovementPage * movementDaysPerPage,
    )
        ...
        <div className="flex items-center justify-between bg-stone-900 border border-stone-800 rounded-xl p-3">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {activeMovementFilters.map((filter, i) => (
              <span
                key={i}
                className="bg-stone-800 text-stone-300 px-2 py-1 rounded-md text-xs border border-stone-700"
              >
                {filter}
              </span>
            ))}
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  <span className="text-stone-500 text-sm">{isCollapsed ? '▼ Görüntüle' : '▲ Daralt'}</span>
                </div>
                    ...
                    <table className="w-full">
                      ...
                      <tbody>
                        {group.items.map((mov) => {
                          ...
                          const timeStr = new Date(mov.created_at).toLocaleTimeString('tr-TR', {
                            hour: '2-digit',
                            minute: '2-digit',
                          })
                          ...
                          return (
                            <tr
                              key={mov.id}
                              className="border-b border-stone-800/30 hover:bg-stone-800/50 transition-colors last:border-0"
                            >
                              ...
                              <td className="px-4 py-2">
                                <span
                                  className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${typeColor} ${typeBg}`}
                                >
                                  {typeLabel}
                                </span>
                              </td>
                              <td className={`px-4 py-2 text-right font-bold ${typeColor}`}>
                                {isGiris || isSayim ? '+' : '-'}
                                {mov.quantity}{' '}
                                <span className="text-xs font-normal opacity-70">{mov.materials?.unit}</span>
                              </td>
                              <td className="px-4 py-2 text-right text-stone-400 text-sm">
                                {mov.unit_price ? formatCurrency(mov.unit_price) : '-'}
                              </td>
                              <td
                                className="px-4 py-2 text-stone-400 text-sm max-w-[200px] truncate"
                                title={mov.note || ''}
                              >
                                {mov.note || '-'}
                              </td>
