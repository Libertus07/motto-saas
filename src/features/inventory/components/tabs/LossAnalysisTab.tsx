...
export function LossAnalysisTab({
  ...
  const filteredZayiMovements = useMemo(() => {
    return [
      ...fireMovements.filter((m) => {
        const matchesSearch =
          m.materials?.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
              ...
              >
                ...
                >
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-stone-500 text-sm">{isExpanded ? '▼' : '▶'}</span>
                  </div>
                </div>
                      ...
                      <tbody>
                        {items.map((mov) => {
                          ...
                          return (
                            <tr
                              key={mov.id}
                              className="border-b border-stone-800/20 hover:bg-stone-800/40 transition-colors last:border-0"
                            >
                              ...
                              <td className="px-4 py-3 text-right font-bold text-red-400">{formatCurrency(loss)}</td>
                              <td
                                className="px-4 py-3 text-stone-400 text-sm max-w-[200px] truncate"
                                title={mov.note || ''}
                              >
                                {mov.note || '-'}
                              </td>
