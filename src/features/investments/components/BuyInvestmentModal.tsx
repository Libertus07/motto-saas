      ...
      <div className="bg-stone-900 border border-stone-700 rounded-2xl p-6 w-full max-w-md shadow-2xl overflow-y-auto max-h-[90vh]">
        ...
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 mb-4 relative overflow-hidden">
          ...
          <div className="relative">
            ...
            />
            <div
              className={`flex items-center justify-center gap-2 py-3 px-4 rounded-xl border border-dashed transition-colors ${isAnalyzing ? 'bg-stone-800 border-stone-600 text-stone-400' : 'bg-stone-900 border-amber-500/50 text-amber-400 hover:bg-amber-500/10'}`}
            >
              {isAnalyzing ? (
                <>
        ...
        <form onSubmit={onSubmit} className="space-y-4">
              ...
              {['gold', 'usd', 'eur', 'real_estate'].map((type) => (
                ...
                >
                  <span className="text-2xl">
                    {type === 'gold' ? '🥇' : type === 'usd' ? '💵' : type === 'eur' ? '💶' : '🏠'}
                  </span>
                  <span className="text-[10px] uppercase">
                    {type === 'gold' ? 'Gr Altın' : type === 'real_estate' ? 'Emlak' : type}
                  </span>
                </button>
              ))}
            </div>
          ...
          {form.asset_type !== 'real_estate' ? (
            ...
            <div>
              <label className="text-stone-400 text-sm mb-1 block text-amber-500/80">
                Gayrimenkul Toplam Maliyeti (₺)
              </label>
              <input
                type="number"
                required
          ...
          <div className="bg-stone-950 p-4 rounded-xl border border-stone-800 flex justify-between items-center">
            ...
            <span className="text-xl font-bold text-red-400">
              {formatCurrency(
                (form.asset_type === 'real_estate' ? 1 : parseFloat(form.quantity) || 0) *
            ...
            >
              ...
              {accounts.map((acc) => (
                <option key={acc.id} value={acc.id}>
                  {acc.name} (Bakiye:{formatCurrency(acc.balance)})
                </option>
              ))}
            </select>
          </div>
          ...
          <button
            ...
            disabled={
              ...
              !form.price_per_unit ||
              !form.account_id ||
              (form.asset_type !== 'real_estate' && !form.quantity)
            }
