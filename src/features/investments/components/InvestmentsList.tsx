  ...
  const renderInvestmentList = (investmentList: EnhancedInvestment[]) => {
    return investmentList.map((inv) => {
      ...
      return (
        <div
          key={inv.id}
          className="bg-stone-900 border border-stone-800 rounded-xl overflow-hidden transition-all duration-200"
        >
          <div
              ...
              <div className="px-5 py-3 border-b border-stone-800 flex flex-wrap gap-2 items-center bg-stone-950/30">
                {inv.isRE && (
                  <>
                    <button
                      onClick={() => onRent(inv)}
                      className="bg-green-600/20 hover:bg-green-600/40 text-green-400 border border-green-600/30 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors flex items-center gap-2"
                    >
                      <span>💰</span> Kira Tahsil Et
                    </button>
                    <button
                      onClick={() => onUpdateValue(inv)}
                      className="bg-stone-800 hover:bg-stone-700 text-white border border-stone-700 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors flex items-center gap-2"
                    >
                ...
                {inv.notes && (
                  <button
                    onClick={() => onNote(inv.notes!)}
                    className="bg-stone-800 hover:bg-stone-700 text-stone-300 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors flex items-center gap-2"
                  >
                    <span>📝</span> Notlar
                  </button>
                )}
                {inv.document_url && (
                  <button
                    onClick={() => onDoc(inv.document_url!)}
                    className="bg-blue-600/20 hover:bg-blue-600/40 text-blue-400 border border-blue-600/30 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors flex items-center gap-2"
                  >
                    <span>📎</span> Belge
                  </button>
                )}
                <button
                  onClick={() => onEdit(inv)}
                  className="bg-stone-800 hover:bg-amber-500/20 text-stone-400 hover:text-amber-400 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors flex items-center gap-2"
                >
                  <span>✏️</span> Düzenle
                </button>
                <button
                  onClick={() => onDelete(inv.id)}
                  className="bg-stone-800 hover:bg-red-500/20 text-stone-400 hover:text-red-400 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors flex items-center gap-2"
                >
                  <span>🗑️</span> Sil
                </button>
              </div>
                    ...
                    {invTransactions.length === 0 ? (
                      ...
                      invTransactions.map((tx) => (
                        <tr key={tx.id} className="hover:bg-stone-800/20 transition-colors">
                              ...
                              className={`px-2 py-1 rounded-md text-xs font-bold border ${
                                tx.transaction_type === 'buy'
                                  ...
                                  : tx.transaction_type === 'rent'
                                    ...
                                    : tx.transaction_type === 'value_update'
                          ...
                          <td className="px-5 py-3 text-stone-300">{tx.notes || '-'}</td>
                          <td className="px-5 py-3 text-right font-bold text-white whitespace-nowrap">
                            {tx.transaction_type === 'value_update' ? '' : tx.transaction_type === 'buy' ? '-' : '+'}
          ...
          >
            <summary className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-3 px-4 cursor-pointer hover:bg-stone-800/50 transition-colors list-none select-none">
              <div className="flex items-center gap-3">
                ...
                <h4 className="text-base font-bold text-amber-500">
                  {groupName} <span className="text-stone-500 text-xs ml-2 font-medium">({items.length} Kayıt)</span>
                </h4>
              </div>
              <div className="flex items-center gap-6 mt-3 sm:mt-0 text-sm pl-7 sm:pl-0">
                ...
                <div
                  className={`font-bold px-2 py-1 rounded-md ${isGroupProfit ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'}`}
                >
                  {isGroupProfit ? 'Kâr: +' : 'Zarar: '}
            ...
            <div className="p-3 pt-0 border-t border-stone-800/50 bg-stone-950/30">
              {groupName === 'Gayrimenkul Mülkleri' || groupBy === 'month' ? (
                <div className="flex flex-col gap-2 mt-3">{renderInvestmentList(items)}</div>
              ) : (
                <div className="flex flex-col gap-3 mt-3">
                  {Object.entries(
                    items.reduce(
                      (acc, curr) => {
                        const dateStr = curr.purchase_date
                          ? formatDate(new Date(curr.purchase_date))
                    ...
                    <details
                      key={monthName}
                      className="group/month bg-stone-900 border border-stone-800/50 rounded-lg overflow-hidden"
                      open
                    >
                      <summary className="flex items-center justify-between p-2 px-4 cursor-pointer hover:bg-stone-800/30 list-none select-none border-b border-stone-800/30">
                        <div className="flex items-center gap-2">
                          <span className="text-stone-600 transition-transform duration-300 group-open/month:rotate-90 text-xs">
                            ▶
                          </span>
