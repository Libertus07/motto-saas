      ...
      if (stockMovs && stockMovs.length > 0) {
        for (const mov of stockMovs) {
          if (mov.material_id && mov.quantity) {
            ...
            await supabase.from('materials').update({ stock_quantity: newQty }).eq('id', mov.material_id)
          }
        }
      }
      ...
      if (supTxs && supTxs.length > 0) {
        for (const tx of supTxs) {
          if (tx.supplier_id && tx.amount) {
            ...
            const newDebt = tx.transaction_type === 'invoice' ? currentDebt - change : currentDebt + change

            await supabase.from('suppliers').update({ total_debt: newDebt }).eq('id', tx.supplier_id)
          }
        }
      }
