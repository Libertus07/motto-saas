...
export default function Giderler() {
  ...
  const handleSubmit = async () => {
    ...
    const finalCategory =
      form.category === 'custom' ? customCategory.trim().toLowerCase().replace(/\s+/g, '-') : form.category
    ...
    if (editingId) {
      ...
      if (oldExp?.expense_date !== payload.expense_date)
        changes.push(`Tarih: ${oldExp?.expense_date} -> ${payload.expense_date}`)
    ...
    logActivity(
      ...
      `${form.name} isimli ${form.amount} TL tutarında gider ${editingId ? 'güncellendi' : 'eklendi'}.`,
      { detay: details },
    )
  }
  ...
  const byCategory = categories
    .map((cat) => ({
      label: cat.label,
      value: cat.value,
      total: recentExpenses.filter((e) => e.category === cat.value).reduce((sum, e) => sum + e.amount, 0),
    }))
    .filter((c) => c.total > 0)
    ...
    [
      {
        ...
        popover: {
          ...
          description:
            'İşletmenizin 30 günlük, günlük ve hatta SAATLİK giderlerini anlık görün. Karar alırken saatlik maliyetinize göre hesap yapın.',
          side: 'bottom',
          align: 'center',
        },
      },
      ...
      },
      {
        ...
        popover: {
          ...
          description:
            'Buradan yeni bir gider faturası işlediğinizde, "Fiyat Motoru" ürünlerinizin birim maliyetlerini anında güncelleyecektir.',
          side: 'bottom',
          align: 'end',
        },
      },
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                ...
                <h3 className="text-3xl font-black text-white tracking-tight">
                  {formatCurrency(monthlyTotal / 30 / 12)}
                </h3>
              </div>
            </div>
          </div>
            ...
            <div className="flex flex-wrap gap-4">
              {byCategory
                ...
                .map((cat, i) => (
                  <div
                    key={cat.value}
                    className="flex items-center gap-2 bg-stone-950/50 px-3 py-1.5 rounded-xl border border-stone-800/50"
                  >
                    ...
                    <span className="text-stone-500 text-sm border-l border-stone-800 pl-2">
              ...
              <HistoryAccordion
                ...
                renderHeaderRight={(group) => {
                  const total = group.items.reduce(
                    (sum, e) => sum + (e.category === 'indirim-ikram' || e.category === 'iade' ? 0 : e.amount),
                    0,
                  )
                  return (
                  ...
                  <div className="space-y-2">
                    {items.map((exp) => {
                      ...
                      return (
                        <div
                          key={exp.id}
                          className="group bg-stone-900 border border-stone-800 rounded-xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:border-amber-500/30 transition-all"
                        >
                          ...
                          <div className="flex items-center justify-between md:justify-end gap-6 border-t border-stone-800 md:border-none pt-3 md:pt-0">
                            <div className="text-right">
                              <p
                                className={`font-bold text-lg ${isDiscount ? 'text-stone-500 line-through' : 'text-rose-400'}`}
                              >
                                {formatCurrency(exp.amount)}
                              </p>
                              {exp.period === 'yearly' && (
                                <p className="text-[10px] text-stone-500">
                                  Aylık Yük: {formatCurrency(exp.amount / 12)}
                                </p>
                              )}
                            </div>
                            <div className="flex items-center gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                              <button
                                ...
                                className="p-2 bg-stone-800/50 hover:bg-blue-500/10 hover:text-blue-400 text-stone-400 rounded-lg transition-colors"
                                aria-label="Düzenle"
                              >
                                ✏️
                              </button>
                              <button
                                ...
                                className="p-2 bg-stone-800/50 hover:bg-rose-500/10 hover:text-rose-400 text-stone-400 rounded-lg transition-colors"
                                aria-label="Sil"
                              >
                                🗑️
                              </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-950/80 backdrop-blur-sm transition-opacity">
          <div className="bg-gradient-to-br from-stone-900 to-stone-950 border border-stone-800 rounded-3xl p-6 md:p-8 w-full max-w-lg shadow-2xl relative animate-in fade-in zoom-in duration-200 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center gap-3 mb-8 pb-6 border-b border-stone-800">
              ...
              <h2 className="text-xl font-bold text-white">{editingId ? 'Gideri Düzenle' : 'Hızlı Gider Ekle'}</h2>
            </div>

            <div className="flex flex-col gap-5 flex-1">
              <div>
                <label className="text-stone-400 text-xs font-bold uppercase tracking-wider mb-2 block">
                  Ne Gideri? (Adı)
                </label>
                <input
                  value={form.name}
              ...
              <div>
                ...
                >
                  ...
                  <option value="custom" className="text-amber-500 font-bold">
                    + Yeni Kategori Ekle
                  </option>
                </select>
