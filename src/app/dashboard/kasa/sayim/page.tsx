...
export default function KasaSayimPage() {
  useAppTour('kasa_sayim', [
    {
      ...
      popover: {
        title: 'Doğru günü seçin',
        description: 'Sayımı kaydetmeden önce işlem gününü kontrol edin; sistem beklentisi bu tarihe göre hesaplanır.',
      },
    },
    {
      element: '#tour-cash-tabs',
      popover: {
        title: 'Sayım ve geçmiş aynı yerde',
        description: 'Gün sonu sayımını tamamlayın veya önceki mutabakatları bu iki sekmeden inceleyin.',
      },
    },
    {
      element: '#tour-cash-count',
      popover: {
        title: 'Kör sayımı girin',
        description: 'Kasadaki nakit, POS ve yemek kartını fiili değerlerle kaydedin; fark otomatik hesaplanır.',
      },
    },
  ])
  ...
  const [adjustmentType, setAdjustmentType] = useState<
    'none' | 'cash_to_credit' | 'credit_to_cash' | 'overcharged_pos_cash_refund'
  >('none')
  ...
  const fetchExpectedTotals = useCallback(async () => {
    ...
    try {
      ...
      const totalExpenses =
        expensesData
          ?.filter((e) => !['indirim-ikram', 'iade'].includes(e.category || ''))
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (existingReconciliation) {
        ...
        const oldTotal =
          Number(existingReconciliation.counted_cash) +
          Number(existingReconciliation.counted_credit_card) +
          Number(existingReconciliation.counted_meal_card || 0)
        const oldVariance =
          oldTotal -
          (Number(existingReconciliation.expected_cash) + Number(existingReconciliation.expected_credit_card))

        if (oldVariance !== variance) {
          changes.push(
            `Kasa Farkı: ${oldVariance > 0 ? '+' : ''}${oldVariance} ₺ -> ${variance > 0 ? '+' : ''}${variance} ₺`,
          )
        }
      ...
      await logActivity(
        'Kasa',
        existingReconciliation ? 'GUNCELLEME' : 'EKLEME',
        `${date} tarihli kasa sayımı ${existingReconciliation ? 'güncellendi' : 'kaydedildi'}.`,
        { detay: details },
      )
  ...
  const getVarianceBadge = () => {
    if (variance > 0)
      return (
        <span className="bg-emerald-500/20 text-emerald-400 px-3.5 py-1.5 rounded-full text-xs font-bold border border-emerald-500/30 shadow-lg shadow-emerald-500/10 flex items-center gap-1.5">
          + {formatCurrency(variance)} Kasa Fazlası
        </span>
      )
    if (variance < 0)
      return (
        <span className="bg-rose-500/20 text-rose-400 px-3.5 py-1.5 rounded-full text-xs font-bold border border-rose-500/30 shadow-lg shadow-rose-500/10 flex items-center gap-1.5">
          - {formatCurrency(Math.abs(variance))} Kasa Açığı
        </span>
      )
    return (
      <span className="bg-amber-500/20 text-amber-400 px-3.5 py-1.5 rounded-full text-xs font-bold border border-amber-500/30 shadow-lg shadow-amber-500/10 flex items-center gap-1.5">
        ✓ Kasa Birebir Denk
      </span>
    )
  }
        ...
        {activeTab === 'history' ? (
                  ...
                  {recentReconciliations.map((rec) => {
                    ...
                    return (
                      <tr key={rec.id} className="hover:bg-stone-800/30 transition-colors">
                        <td className="py-3 px-4 font-bold text-stone-200">📅 {rec.date}</td>
                        <td className="py-3 px-4 font-semibold text-amber-400">{formatCurrency(rec.counted_cash)}</td>
                        ...
                        <td className="py-3 px-4 font-bold">
                          <span
                            className={
                              totDiff > 0 ? 'text-emerald-400' : totDiff < 0 ? 'text-rose-400' : 'text-amber-400'
                            }
                          >
                            {totDiff > 0 ? '+ ' : ''}
          ...
          <main className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-7 space-y-6">
              <div
                id="tour-cash-count"
                className="bg-stone-900/80 backdrop-blur-xl border border-stone-800/80 rounded-3xl p-6 lg:p-8 shadow-2xl relative overflow-hidden space-y-6"
              >
                <div className="flex items-center justify-between">
                  <h2 className="text-xl font-black text-white flex items-center gap-2.5">
                    <span className="w-7 h-7 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-400 flex items-center justify-center text-xs">
                      1
                    </span>
                    ...
                    <div className="bg-stone-950/90 border border-amber-500/30 rounded-2xl p-5 space-y-4 animate-fade-in shadow-2xl">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-stone-400">
                              Adetleri girin, nakit tutarı otomatik toplansın.
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
                        {DENOMINATIONS.map((d) => (
                          <div
                            key={d.value}
                            className="bg-stone-900/90 border border-stone-800 rounded-xl p-2.5 space-y-1.5"
                          >
            ...
            <div className="lg:col-span-5 space-y-6">
              ...
              <div className="bg-stone-900/80 backdrop-blur-xl border border-stone-800/80 rounded-3xl p-6 lg:p-8 shadow-2xl relative overflow-hidden space-y-5">
                <div className="flex items-center justify-between">
                  <h2 className="text-xl font-black text-white flex items-center gap-2.5">
                    <span className="w-7 h-7 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 flex items-center justify-center text-xs">
                      2
                    </span>
                    Sistem Beklentisi
                  </h2>
              ...
              <div
                className={`border rounded-3xl p-6 lg:p-8 shadow-2xl transition-all space-y-5 ${
                  variance === 0
              ...
              >
                  ...
                  {isMovementFound && (
                    <div className="grid grid-cols-2 gap-3 w-full mt-2">
                      <div className="bg-stone-950/80 p-3 rounded-2xl border border-stone-800/80 text-center">
                        ...
                        <div
                          className={`font-bold text-sm ${cashVariance === 0 ? 'text-amber-400' : cashVariance > 0 ? 'text-emerald-400' : 'text-rose-400'}`}
                        >
                          {cashVariance > 0 ? '+ ' : ''}
                      ...
                      <div className="bg-stone-950/80 p-3 rounded-2xl border border-stone-800/80 text-center">
                        ...
                        <div
                          className={`font-bold text-sm ${creditVariance === 0 ? 'text-amber-400' : creditVariance > 0 ? 'text-emerald-400' : 'text-rose-400'}`}
                        >
                          {creditVariance > 0 ? '+ ' : ''}
                          {formatCurrency(creditVariance)}
                        </div>
                      </div>
                    </div>
                  )}
                ...
                >
                  {saving
                    ...
                    : existingReconciliation
                      ? '💾 Mutabakatı Güncelle'
                      : '🔒 Günü Kapat ve Mutabakatı Kaydet'}
                </button>
              </div>
            </div>
          </main>
