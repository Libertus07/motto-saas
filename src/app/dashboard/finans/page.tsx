...
export default function FinansPage() {
  useAppTour('finans', [
    {
      ...
      popover: {
        title: 'İşlem başlatın',
        description: 'Kasa sayımına geçin veya modüllerden gelmeyen gelir–gideri manuel olarak kaydedin.',
      },
    },
    {
      element: '#tour-finance-kpis',
      popover: {
        title: 'Likiditeyi takip edin',
        description: 'Kasa, banka ve aylık net akış tek bir finansal özet içinde sunulur.',
      },
    },
    {
      element: '#tour-finance-accounts',
      popover: {
        title: 'Hesap hareketlerini yönetin',
        description: 'Hesap bakiyelerini ve işlem geçmişini kontrol ederek nakit akışını izleyin.',
      },
    },
  ])
  ...
  const filteredMovements = useMemo(() => {
    return movements.filter((m) => {
      ...
      // Source filter
      if (filterSource !== 'all') {
        if (filterSource === 'z_report' && !['z_report', 'z_report_group', 'reconciliation'].includes(m.source_type))
          return false
  ...
  const handleDeleteMovement = async (move: AccountMovement) => {
    ...
    try {
      ...
      const { error: delError } = await supabase.from('account_movements').delete().eq('id', move.id)
  ...
  const handleRedirectToModule = async (move: AccountMovement) => {
    ...
    if (
      move.source_type === 'z_report' ||
      move.source_type === 'reconciliation' ||
      move.source_type === 'z_report_group'
    ) {
      ...
      <main className="max-w-7xl mx-auto p-6 space-y-8 mt-4">
        ...
        <div id="tour-finance-kpis" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-stone-900/80 rounded-2xl p-5 border border-stone-800/80 hover:border-stone-700/80 shadow-md transition-all group relative overflow-hidden">
            ...
            <h3 className="text-2xl font-bold text-emerald-400 tracking-tight">{formatCurrency(totalCash)}</h3>
            <p className="text-xs text-stone-500 mt-2">Restoran içi fiziksel nakit</p>
          </div>
          ...
          <div className="bg-stone-900/80 rounded-2xl p-5 border border-stone-800/80 hover:border-stone-700/80 shadow-md transition-all group relative overflow-hidden">
            ...
            <h3 className="text-2xl font-bold text-blue-400 tracking-tight">{formatCurrency(totalBank)}</h3>
            <p className="text-xs text-stone-500 mt-2">Banka vadesiz & POS bakiyesi</p>
          </div>
          ...
          <div className="bg-stone-900/80 rounded-2xl p-5 border border-stone-800/80 hover:border-stone-700/80 shadow-md transition-all group relative overflow-hidden">
            <div className="flex justify-between items-start mb-3">
              ...
              <span
                className={`p-2 rounded-lg border text-lg ${monthlyNetFlow >= 0 ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border-rose-500/20'}`}
              >
                {monthlyNetFlow >= 0 ? '📈' : '📉'}
              </span>
            </div>
            <h3
              className={`text-2xl font-bold tracking-tight ${monthlyNetFlow >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}
            >
              {monthlyNetFlow >= 0 ? '+ ' : ''}
              {formatCurrency(monthlyNetFlow)}
            </h3>
            <p className="text-xs text-stone-500 mt-2">Ay başından itibaren giriş-çıkış farkı</p>
          </div>
        </div>
        ...
        <section id="tour-finance-accounts" aria-labelledby="accounts-heading">
          <div className="flex justify-between items-center mb-4">
            ...
            <span className="text-xs text-stone-500 font-mono">{accounts.length} Aktif Hesap</span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {loading
              ? Array.from({ length: 4 }).map((_, idx) => (
                  <div
                    key={idx}
                    className="bg-stone-900/50 border border-stone-800 rounded-2xl p-6 h-36 animate-pulse"
                  ></div>
                ))
              : accounts.map((acc) => {
                  ...
                  return (
                    <div
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter' || e.key === ' ') setSelectedAccount(acc)
                      }}
                    ...
                    >
                      ...
                      <p
                        className={`text-sm font-semibold mb-1 truncate ${isSelected ? 'text-amber-200' : 'text-stone-400 group-hover:text-stone-200'}`}
                      >
                        {acc.name}
                      </p>

                      <h3
                        className={`text-2xl font-black tracking-tight ${isSelected ? 'text-amber-400' : 'text-white'}`}
                      >
              ...
              {movements.length === 0 ? (
                ...
                <HistoryAccordion
                  groups={Object.entries(groupedMovements).map(([dateKey, groupMoves]) => ({
                    ...
                    icon: (
                      <span className="text-xl">{dateKey === 'Bugün' ? '📅' : dateKey === 'Dün' ? '⏱️' : '🗓️'}</span>
                    ),
                    items: groupMoves,
                  }))}
                            ...
                            return (
                              ...
                              >
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
                                  <span
                                    className={`inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold border ${sourceBadgeClass}`}
                                  >
                                    {sourceLabel}
                                  </span>
                                </td>
                                ...
                                <td className="px-6 py-4 text-right whitespace-nowrap">
                                  <span
                                    className={`inline-flex items-center font-bold px-3 py-1.5 rounded-xl border text-sm ${
                                      isGiris
        ...
        >
          <div className="bg-stone-900 border border-stone-700/80 rounded-2xl p-6 w-full max-w-md shadow-2xl relative overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleManualEntry} className="space-y-4">
              <div>
                <label className="text-stone-400 text-xs font-bold uppercase tracking-wider mb-1.5 block">
        ...
        >
          ...
          >
            ...
            <div className="text-center mb-6 mt-2">
              <div
                className={`w-16 h-16 mx-auto rounded-2xl flex items-center justify-center text-3xl mb-3 shadow-lg ${
              ...
              <span
                ...
                className="text-stone-400 text-xs font-bold uppercase tracking-widest block mb-1"
              >
                Dijital İşlem Dekontu
              </span>
              <h2
                className={`text-3xl font-black tracking-tight ${selectedMovement.movement_type === 'giris' ? 'text-emerald-400' : 'text-rose-400'}`}
              >
                {selectedMovement.movement_type === 'giris' ? '+ ' : '- '}
            ...
            <div className="space-y-4">
              <div className="bg-stone-950/70 p-4 rounded-2xl border border-stone-800/80 space-y-3">
                ...
                <div className="flex justify-between items-center pb-2.5 border-b border-stone-800/60 text-xs">
                  ...
                  <span
                    className={`font-bold ${selectedMovement.movement_type === 'giris' ? 'text-emerald-400' : 'text-rose-400'}`}
                  >
                ...
                <div className="flex justify-between items-center pb-2.5 border-b border-stone-800/60 text-xs">
                  ...
                  <span className="text-amber-400 font-bold">
                    {selectedMovement.source_type === 'z_report'
                      ? 'Z-Raporu'
                      : selectedMovement.source_type === 'z_report_group'
                        ? 'Z-Raporu & Kasa Sayımı'
                        : selectedMovement.source_type === 'supplier_payment'
                          ? 'Tedarikçi Ödemesi'
                          : selectedMovement.source_type === 'expense'
                            ? 'Masraf/Gider'
                            : 'Manuel İşlem'}
                  </span>
                </div>
              ...
              <div className="bg-stone-950/70 p-4 rounded-2xl border border-stone-800/80">
                <span className="text-stone-500 text-xs font-bold uppercase tracking-wider block mb-1.5">
                  Açıklama Notu
                </span>
                <p className="text-stone-200 text-xs leading-relaxed font-medium">{selectedMovement.description}</p>

                {selectedMovement.z_details && (
                  <div className="mt-4 pt-4 border-t border-stone-800/80 space-y-2.5">
                    <div className="flex justify-between items-center text-xs">
                      <span className="text-stone-400 flex items-center gap-1.5">
                        🟢 <span>Hasılat Toplamı</span>
                      </span>
                      <span className="text-emerald-400 font-bold">
                        + {formatCurrency(selectedMovement.z_details.hasilat)}
                      </span>
                    </div>
                    <div className="flex justify-between items-center text-xs">
                      <span className="text-stone-400 flex items-center gap-1.5">
                        🔴 <span>Kasadan Giderler</span>
                      </span>
                      <span className="text-rose-400 font-bold">
                        - {formatCurrency(selectedMovement.z_details.gider)}
                      </span>
                    </div>
                    {selectedMovement.z_details.acikFazla !== 0 && (
                      <div className="flex justify-between items-center text-xs">
                        <span className="text-stone-400 flex items-center gap-1.5">
                          🟠 <span>Kasa {selectedMovement.z_details.acikFazlaTipi}</span>
                        </span>
                        <span
                          className={`${selectedMovement.z_details.acikFazla > 0 ? 'text-emerald-400' : 'text-rose-400'} font-bold`}
                        >
                          {selectedMovement.z_details.acikFazla > 0 ? '+ ' : '- '}
                          {formatCurrency(Math.abs(selectedMovement.z_details.acikFazla))}
                        </span>
                      </div>
                    )}
                    <div className="flex justify-between items-center pt-2.5 border-t border-stone-800/80 mt-2">
                      ...
                      <span className="text-amber-400 font-black text-lg">
                        {selectedMovement.z_details.net > 0 ? '+ ' : ''}
                        {formatCurrency(selectedMovement.z_details.net)}
                      </span>
                    </div>
                  </div>
                )}
