...
export default function YatirimlarPage() {
      ...
      reader.onloadend = async () => {
        ...
        try {
          ...
          if (errorMsg === 'The string did not match the expected pattern.') {
            errorMsg =
              'Tarayıcı kaynaklı bir hata oluştu. Yüklediğiniz fotoğrafın formatı (HEIC vb.) desteklenmiyor olabilir.'
          }
  ...
  const handleFileUpload = async <T extends { document_url?: string }>(
  ...
  const enhancedInvestments = investments.map((inv) => {
    ...
    const invRentIncome = transactions
      .filter((t) => t.investment_id === inv.id && t.transaction_type === 'rent')
      .reduce((t, tx) => t + Number(tx.total_amount), 0)
    ...
    [
      {
        ...
        popover: {
          ...
          description:
            'Cebinizden çıkan toplam para, anlık canlı kurlara göre bugünkü değeriniz ve kazancınız hesaplanır.',
          side: 'bottom',
          align: 'center',
        },
      },
      {
        ...
        popover: {
          ...
          description:
            'Varlıklarınızı altın, döviz gibi türüne veya aldığınız aya göre gruplayarak detaylı analiz edebilirsiniz.',
          side: 'top',
          align: 'start',
        },
      },
  ...
  return (
    <div className="min-h-full bg-stone-950 text-white pb-20">
      <header className="mb-8 p-6 pb-0 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-amber-500 flex items-center gap-3">📈 Yatırımlar ve Portföy</h1>
          <p className="text-stone-400 mt-2 max-w-2xl">
            İşletmenizin varlıklarını döviz, altın ve gayrimenkul olarak koruyun; anlık değerlerini ve kira getirilerini
            takip edin.
          </p>
        </div>
      ...
      <main className="p-6 pt-0 space-y-6">
        ...
        <div id="tour-inv-kpis" className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div
            className={`border rounded-2xl p-6 relative overflow-hidden flex flex-col justify-between ${totalProfit >= 0 ? 'bg-green-950/20 border-green-500/30' : 'bg-red-950/20 border-red-500/30'}`}
          >
            <div className="absolute -right-4 -top-4 text-7xl opacity-5">📈</div>
            <div>
              ...
              <h2
                className={`text-3xl font-bold mb-2 flex flex-wrap items-center gap-2 ${totalProfit >= 0 ? 'text-green-400' : 'text-red-400'}`}
              >
                {totalProfit >= 0 ? '+' : ''}
        ...
        <div
          id="tour-inv-tools"
          className="flex flex-col sm:flex-row justify-between items-start sm:items-center mt-8 mb-4 gap-4"
        >
          ...
          <div className="flex flex-wrap items-center gap-2 bg-stone-900 border border-stone-800 p-2 rounded-xl">
            ...
            <button
              ...
              title={
                ui.sortOrder === 'desc'
                  ? 'Azalan Sıralama (Z-A, Yeni-Eski, Yüksek-Düşük)'
                  : 'Artan Sıralama (A-Z, Eski-Yeni, Düşük-Yüksek)'
              }
            >
