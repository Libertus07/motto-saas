...
export default function TedarikciGecmisi() {
  ...
  const handleDelete = async (group: GroupedReceipt) => {
    if (!group.id) {
      await showAlert(
        'Bu fiş eski bir versiyona ait (Batch ID eksik) olduğu için otomatik olarak geri alınamaz. Lütfen manuel olarak stok ve cari düzeltmesi yapın.',
        'warning',
      )
      return
    }
    ...
    try {
      ...
      await logActivity(
        'Tedarikçi Geçmişi',
        'SILME',
        `${formattedDate} tarihli ${group.supplierName} fişi silindi. Stoklar ve cari bakiyeler geri alındı.`,
        { batchId: group.id },
      )
      ...
      <main className="p-6 max-w-5xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
          <div>
            ...
            <p className="text-stone-400 text-sm max-w-xl">
              Yapay zeka ile yüklediğiniz tedarikçi fişleri kategorize edilmiştir. Fişleri aylara veya tedarikçilere
        ...
        {loading ? (
          ...
          <HistoryAccordion<GroupedReceipt>
            ...
            defaultExpandedIds={
              displayData.length > 0
                ? ['supplierName' in displayData[0] ? displayData[0].supplierName : displayData[0].monthKey]
                : []
            }
            renderHeaderRight={(group) => {
              const mainGroup = displayData.find(
                (g) => ('supplierName' in g ? g.supplierName : g.monthKey) === group.id,
              )
              return (
                        ...
                        >
                          ...
                          <div>
                            <h4 className="font-bold text-stone-200">
                              {groupBy === 'supplier' ? dateStr : receipt.supplierName}
                            </h4>
                            <p className="text-xs text-stone-500 mt-0.5">
                              {groupBy === 'supplier' ? 'Fiş Tarihi' : dateStr} • {receipt.totalItems} kalem ürün
                            </p>
                          </div>
                        </button>
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <button
                              ...
                              className={`p-2 rounded-lg transition-colors border ${
                                !receipt.batchId
                                  ? 'bg-stone-800 border-stone-800 text-stone-600 cursor-not-allowed opacity-50'
