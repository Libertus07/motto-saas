...
export default function GecmisRaporlar() {
  ...
  const fetchSales = useCallback(async () => {
    ...
    ;(data ?? []).forEach((item: SaleItem) => {
      ...
      item.product_name =
        item.product_id && productMap[item.product_id] ? productMap[item.product_id] : 'Bilinmeyen Ürün'
  ...
  const handleDelete = async (group: GroupedSale) => {
    if (!group.batchId) {
      await showAlert(
        'Bu Z-Raporu eski bir versiyona ait (Batch ID eksik) olduğu için otomatik olarak geri alınamaz.',
        'warning',
      )
      return
    }
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
          <div>
            ...
            <p className="text-stone-400 text-sm max-w-xl">
              Sisteme yüklediğiniz Z-Raporları aylara göre kategorize edilmiştir. İçerisindeki günleri görebilir,
                              ...
                              <tbody className="divide-y divide-stone-800/50">
                                {day.items.map((item) => (
                                  <tr key={item.id} className="hover:bg-stone-800/50 transition-colors">
                                    <td className="px-5 py-2.5 font-medium text-stone-300">{item.product_name}</td>
