...
export default function ZRaporuYukle() {
  ...
  const matchExpenseCategory = (expenseName: string): string => {
    ...
    if (name.includes('internet') || name.includes('telefon') || name.includes('ttnet') || name.includes('turkcell'))
      return 'internet'
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (errorMsg === 'The string did not match the expected pattern.') {
        errorMsg =
          'Tarayıcı kaynaklı bir hata oluştu. Yüklediğiniz fotoğrafın formatı (HEIC vb.) veya boyutu desteklenmiyor olabilir. Lütfen ekran görüntüsü alıp veya kırparak tekrar deneyin.'
      } else if (errorMsg.includes('Failed to fetch')) {
        errorMsg =
          'Sunucuya bağlanılamadı. Fotoğraf boyutu çok büyük olabilir (Vercel 4.5MB limiti). Lütfen fotoğrafı küçültüp tekrar deneyin.'
      }
    ...
    try {
        ...
        if (accountMovements.length > 0) {
          ...
          for (const m of accountMovements) {
            ...
            if (currAcc) {
              ...
              await supabase
                .from('accounts')
                .update({ balance: Number(currAcc.balance) + amountChange })
                .eq('id', m.account_id)
            }
          }
        }
      ...
      if (matchedSales.length > 0) {
        ...
        const { data: prodIngs } = await supabase
          ...
          .in(
            'product_id',
            matchedSales.map((s) => s.matchedProductId),
          )
        ...
        if (movementInserts.length > 0) {
          ...
          const { data: currentMats } = await supabase
            .from('materials')
            .select('*')
            .in('id', Object.keys(stockDeductions))
      ...
      logActivity(
        'Z-Raporu',
        'EKLEME',
        `${reportDate} tarihli Z-Raporu sisteme eklendi ve toplam ${matchedSales.length} kalem satıldı.`,
        {
          batchId,
  ...
  return (
    <div className="min-h-full bg-stone-950 text-white">
      <header className="bg-stone-900 border-b border-stone-800 px-6 py-4 flex items-center gap-3">
        <button onClick={() => router.push('/dashboard/raporlar')} className="text-stone-400 hover:text-white">
          ← Geri
        </button>
          ...
          <div className="space-y-6">
            <div className="bg-stone-900 border border-stone-800 rounded-xl p-8 text-center border-dashed">
              ...
              <p className="text-stone-400 mb-6 max-w-md mx-auto">
                Z Raporu görseli, PDF, XML veya JSON formatında satış raporunuzu yükleyebilirsiniz. Yapay zeka satışları
                okuyacak ve stokları düşecek.
              </p>
              ...
              <div className="text-left md:text-right w-full md:w-auto flex flex-col sm:flex-row gap-4 justify-end flex-wrap mt-4 md:mt-0">
                    ...
                    onChange={(e) =>
                      setParsedData({
                        ...
                        payment_methods: {
                          ...
                          credit_card: parsedData.payment_methods?.credit_card || 0,
                          other: parsedData.payment_methods?.other || 0,
                        },
                      })
                    }
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setParsedData({
                        ...parsedData,
                        payment_methods: {
                          cash: parsedData.payment_methods?.cash || 0,
                ...
                <div>
                  ...
                  <div className="text-2xl font-bold text-green-400 bg-stone-900 px-4 py-2 rounded-lg border border-green-500/30 inline-block">
                    {formatCurrency(
                      (parsedData.payment_methods?.cash || 0) -
                        (parsedData.expenses || []).reduce((acc, exp) => acc + (Number(exp.amount) || 0), 0),
                    )}
                  </div>
                </div>
              </div>
            ...
            <div className="bg-stone-900 rounded-xl border border-stone-800 overflow-hidden">
              <div className="p-4 border-b border-stone-800 bg-stone-800/50">
                ...
                <p className="text-sm text-stone-400">
                  Lütfen sistemdeki ürünlerinizle doğru eşleştiğinden emin olun. Eşleşmeyenlerin stoğu düşülemez.
                </p>
              </div>
                              ...
                              <button
                                onClick={() =>
                                  setNewProductModal({
                                    ...
                                    price:
                                      item.quantity > 0 ? Number((item.total_price / item.quantity).toFixed(2)) : 0,
                                    category: 'Genel',
                                  })
                                }
            ...
            <div className="bg-stone-900 rounded-xl border border-stone-800 overflow-hidden mt-6">
              <div className="p-4 border-b border-stone-800 bg-stone-800/50 flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-stone-400">
                    Kasadan yapılan masrafları veya uygulanan indirimleri/ikramları buraya ekleyin.
                  </p>
                </div>
              </div>
              <div className="overflow-x-auto">
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-stone-900 border border-stone-700 rounded-2xl p-6 w-full max-w-md">
            ...
            <p className="text-stone-400 text-sm mb-6">
              &quot;{newProductModal.name}&quot; sistemde bulunamadı. Yeni bir ürün olarak kataloga ekleyebilirsiniz.
            </p>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-stone-500 mt-1">
                  Olmayan bir kategori yazarak sisteme anında ekleyebilirsiniz.
                </p>
              </div>
              <div>
