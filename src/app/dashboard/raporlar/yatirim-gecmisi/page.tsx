        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
          <div>
            ...
            <p className="text-stone-400 text-sm max-w-xl">
              Yüklediğiniz veya sisteme manuel olarak girdiğiniz tüm altın, döviz ve gayrimenkul belgeleri ay bazlı
              olarak listelenir.
            </p>
          </div>
            ...
            renderContent={(items) => (
              <div className="p-4 space-y-3">
                {items.map((inv) => (
                  <div
                    key={inv.id}
                    className="bg-stone-900 border border-stone-800 rounded-lg p-4 flex flex-col md:flex-row md:items-center justify-between gap-4"
                  >
                    <div className="flex items-center gap-3">
                      <div className="text-3xl">
                        {inv.investments?.asset_type === 'eur' || inv.investments?.asset_type === 'usd'
                          ? '💵'
                          : inv.investments?.asset_type === 'gold'
                            ? '🪙'
                            : '🏢'}
                      </div>
                      <div>
                        ...
                        <p className="text-stone-400 text-sm">
                          {formatDate(new Date(inv.transaction_date || new Date().toISOString()))} • {inv.quantity}{' '}
                          {inv.investments?.asset_type === 'gold'
                            ? 'Gram'
                            : inv.investments?.asset_type === 'real_estate'
                              ? 'Adet'
                              : 'Birim'}
                        </p>
                      </div>
                    ...
                    <div className="flex flex-wrap items-center gap-6">
                      ...
                      {inv.transaction_type === 'buy' && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation()
                            handleDelete(inv.id, inv.investments?.name || 'Yatırım İşlemi', Number(inv.total_amount))
                          }}
