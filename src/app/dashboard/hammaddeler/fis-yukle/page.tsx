  ...
  return (
    <div className="min-h-full bg-stone-950 text-white">
      ...
      <header className="bg-stone-900 border-b border-stone-800 px-6 py-4 flex items-center gap-3">
        <button onClick={() => router.back()} className="text-stone-400 hover:text-white">
          ← Geri
        </button>
      ...
      <main className="p-6 max-w-3xl mx-auto">
        ...
        {step === 'upload' && (
          <div className="space-y-6">
            ...
            {error && <div className="bg-red-900/30 border border-red-500 rounded-xl p-4 text-red-400">{error}</div>}
          </div>
        )}
        ...
        {step === 'review' && (
          <div className="space-y-6">
            {error && (
              <div className="bg-red-900/30 border border-red-500 rounded-xl p-4 text-red-400 mb-6">{error}</div>
            )}
            {parsedSupplier && (
              <div className="bg-stone-900 border border-amber-500/50 rounded-xl p-5 mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-b border-stone-800/80 pb-5 mb-5">
                  <div>
                    <label className="text-stone-400 text-xs font-semibold mb-2 block uppercase tracking-wider">
                      Tedarikçi Adı (Seç veya Yeni Yaz)
                    </label>
                    <input
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                  ...
                  <div className="bg-stone-950 rounded-xl p-4 border border-stone-800 flex flex-col justify-center">
                    <span className="text-stone-500 text-xs font-semibold mb-1 uppercase tracking-wider">
                      Önceki Bakiye (Borç)
                    </span>
                  ...
                  <div className="bg-stone-950 rounded-xl p-4 border border-stone-800 focus-within:border-red-500/50 transition-colors flex flex-col justify-center">
                    ...
                    <div className="flex items-center">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setParsedSupplier({ ...parsedSupplier, totalAmount: parseFloat(e.target.value) || 0 })
                        }
                  ...
                  <div className="bg-stone-950 rounded-xl p-4 border border-stone-800 flex flex-col justify-center relative group">
                    ...
                    {parsedSupplier.statedDebt != null && (
                      <div className="absolute top-[110%] left-0 w-full min-w-[200px] bg-blue-950/95 border border-blue-500/50 rounded-xl p-3 text-xs text-blue-200 shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-20 backdrop-blur-md">
                        📄 Faturada yazan güncel bakiye:
                        <br />
                        <strong className="text-sm text-blue-100 mt-1 block">
                          {formatCurrency(parsedSupplier.statedDebt)}
                        </strong>
                        <span className="text-[10px] text-blue-400/60 mt-1 block leading-tight">
                          (Sistemdeki bakiye ile kıyaslayabilirsiniz)
                        </span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="bg-amber-500/5 rounded-xl p-4 border border-amber-500/30 focus-within:border-amber-500/60 transition-colors flex flex-col justify-center">
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span className="text-[9px] text-amber-500/50 uppercase tracking-wider px-1.5 py-0.5 bg-amber-500/10 rounded hidden xl:block">
                        Sıfırsa borca yazılır
                      </span>
                    </div>
                    <div className="flex items-center">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setParsedSupplier({ ...parsedSupplier, paidAmount: parseFloat(e.target.value) || 0 })
                        }
                ...
                <div className="flex items-center gap-3 pt-2">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      {item.isNew ? (
                        <span className="text-xs bg-green-900 text-green-400 px-2 py-1 rounded-full whitespace-nowrap">
                          Yeni Kayıt
                        </span>
                      ) : (
                        <span className="text-xs bg-blue-900 text-blue-400 px-2 py-1 rounded-full whitespace-nowrap">
                          Stok Ekle
                        </span>
                      )}
                      <button
                        ...
                        <div className="w-full h-9 flex items-center bg-stone-900 border border-stone-800 rounded px-2 text-sm font-bold text-amber-400 overflow-hidden text-ellipsis whitespace-nowrap">
                          {formatCurrency(
                            (Number.isNaN(item.quantity) ? 0 : item.quantity) *
                              (Number.isNaN(item.unitPrice) ? 0 : item.unitPrice),
                          )}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-2">
                      {(item.unit.toLowerCase() === 'kg' ||
                        ...
                        >
                          ⚖️{' '}
                          {item.unit.toLowerCase() === 'kg' || item.unit.toLowerCase() === 'kilogram' ? 'Gram' : 'Ml'}
                          &apos;a Dönüştür
                        </button>
                      )}
                      {(item.unit.toLowerCase() === 'kutu' ||
                        item.unit.toLowerCase() === 'koli' ||
                        item.unit.toLowerCase() === 'paket' ||
            ...
            <div className="flex justify-center mt-4">
              <button
                ...
                className="text-amber-400 hover:text-amber-300 border border-amber-400/50 hover:bg-amber-950 px-6 py-2 rounded-lg font-bold transition-colors"
              >
                + Yeni Kalem Ekle
              </button>
            </div>
