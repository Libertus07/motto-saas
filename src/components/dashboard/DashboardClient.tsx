...
export function DashboardClient() {
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      try {
        ...
        devError('Kurlar çekilemedi', e)
      }

      // 3. Yatırımların canlı kurla çarpılması
      const invList =
        (rpcStats.investmentsList as { asset_type: string; quantity: number | string; average_cost: number }[]) || []
  ...
  const modules = [
    { icon: '🧪', title: 'Hammaddeler', desc: 'Malzeme listesi ve fiyatlar', path: '/dashboard/hammaddeler' },
    {
      ...
      desc: 'Kendi üretiminiz pastalar/tepsiler',
  ...
  return (
    <>
        ...
        <div id="tour-dash-alerts" className="flex flex-col gap-4 mb-8">
          {stats.criticalStockCount > 0 && (
            <div className="bg-red-500/10 border border-red-500/20 backdrop-blur-md rounded-3xl p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-[0_0_30px_rgba(239,68,68,0.1)] relative overflow-hidden">
              ...
              <div className="flex items-start md:items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-red-300/80 text-sm mt-1">
                    <span className="font-bold text-white">{stats.criticalStockCount} adet</span> hammaddenin stoğu
              ...
              <a
                ...
                aria-label="Toptancıya WhatsApp mesajı gönder"
              ...
              >
                <span aria-hidden="true">📱</span> Toptancıya Gönder
              </a>
            </div>
          )}
          ...
          {stats.lowMarginProducts > 0 && (
            <div className="bg-orange-500/10 border border-orange-500/20 backdrop-blur-md rounded-3xl p-6 flex flex-col md:flex-row items-center gap-4 shadow-[0_0_30px_rgba(249,115,22,0.1)] relative overflow-hidden">
              ...
              <div
                className="w-12 h-12 rounded-2xl bg-orange-500/20 flex items-center justify-center text-orange-500 text-2xl shadow-inner"
                aria-hidden="true"
              >
                📉
              </div>
              <div>
                ...
                <p className="text-orange-300/80 text-sm mt-1">
                  <span className="font-bold text-white">{stats.lowMarginProducts} adet</span> ürünün kar marjı
                  hedeflenen %{stats.targetMargin}&apos;in altında. Fiyat motorunu kontrol edin.
                </p>
              </div>
            </div>
          )}
      ...
      <h2 className="text-lg font-bold mb-6 text-stone-300 flex items-center gap-2 uppercase tracking-wider">
        <span aria-hidden="true">💼</span> Finansal Özet
      </h2>
      <div id="tour-dash-financials" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div className="bg-gradient-to-br from-stone-900 to-stone-950 border border-stone-800 rounded-3xl p-6 relative overflow-hidden group hover:border-amber-500/30 transition-all shadow-xl">
          <div
            className="absolute -right-4 -bottom-4 text-8xl opacity-[0.03] group-hover:scale-110 group-hover:rotate-6 transition-transform duration-500"
            aria-hidden="true"
          >
            💵
          </div>
          <div
            className="w-12 h-12 rounded-2xl bg-emerald-500/10 flex items-center justify-center text-emerald-500 text-xl border border-emerald-500/20 mb-4"
            aria-hidden="true"
          >
            💵
          </div>
          <p className="text-stone-400 text-xs font-bold uppercase tracking-wider mb-1">Nakit Kasa</p>
          <p className="text-2xl font-black text-white">{loading ? '...' : formatCurrency(stats.totalCash)}</p>
        </div>
        ...
        <div className="bg-gradient-to-br from-stone-900 to-stone-950 border border-stone-800 rounded-3xl p-6 relative overflow-hidden group hover:border-blue-500/30 transition-all shadow-xl">
          ...
          <div
            className="w-12 h-12 rounded-2xl bg-blue-500/10 flex items-center justify-center text-blue-500 text-xl border border-blue-500/20 mb-4"
            aria-hidden="true"
          >
            🏦
          </div>
          <p className="text-stone-400 text-xs font-bold uppercase tracking-wider mb-1">Banka Hesapları</p>
          <p className="text-2xl font-black text-white">{loading ? '...' : formatCurrency(stats.totalBank)}</p>
        </div>

        <div
          tabIndex={0}
          role="button"
          aria-label="Yatırımlar sayfasına git"
          onClick={() => router.push('/dashboard/yatirimlar')}
          onKeyDown={(e) => e.key === 'Enter' && router.push('/dashboard/yatirimlar')}
          className="bg-gradient-to-br from-stone-900 to-stone-950 border border-stone-800 rounded-3xl p-6 relative overflow-hidden group hover:border-purple-500/30 transition-all shadow-xl cursor-pointer"
        >
          <div
            className="absolute -right-4 -bottom-4 text-8xl opacity-[0.03] group-hover:scale-110 group-hover:rotate-6 transition-transform duration-500"
            aria-hidden="true"
          ...
          <div
            className="w-12 h-12 rounded-2xl bg-purple-500/10 flex items-center justify-center text-purple-500 text-xl border border-purple-500/20 mb-4"
            aria-hidden="true"
          >
            📈
          </div>
          ...
          <p className="text-2xl font-black text-purple-400">
            {loading ? '...' : formatCurrency(stats.totalInvestments)}
          </p>
        </div>
        ...
        <div className="bg-gradient-to-br from-stone-900 to-amber-950/20 border border-amber-500/30 rounded-3xl p-6 relative overflow-hidden group shadow-[0_0_30px_rgba(245,158,11,0.1)]">
          <div
            className="absolute -right-4 -bottom-4 text-8xl opacity-10 group-hover:scale-110 group-hover:rotate-6 transition-transform duration-500"
            aria-hidden="true"
          >
            👑
          </div>
          ...
          <div
            className="w-12 h-12 rounded-2xl bg-amber-500/20 flex items-center justify-center text-amber-500 text-xl border border-amber-500/30 mb-4 shadow-inner"
            aria-hidden="true"
          >
            👑
          </div>
          <p className="text-amber-400/80 text-xs font-bold uppercase tracking-wider mb-1">Toplam Net Varlık</p>
          <p className="text-3xl font-black text-amber-400 tracking-tight drop-shadow-md">
            {loading ? '...' : formatCurrency(stats.totalCash + stats.totalBank + stats.totalInvestments)}
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-lg font-bold text-stone-300 flex items-center gap-2 uppercase tracking-wider">
          <span aria-hidden="true">📊</span> Operasyonel Analiz
        </h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-12">
        ...
        <div
          id="tour-dash-pnl"
          className="lg:col-span-2 bg-gradient-to-br from-stone-900 to-stone-950 border border-stone-800 rounded-3xl p-8 relative overflow-hidden shadow-2xl"
        >
          <div
            className={`absolute -top-32 -right-32 w-64 h-64 blur-[100px] opacity-20 transition-all duration-1000 ${stats.netProfit >= 0 ? 'bg-emerald-500' : 'bg-red-500'}`}
          />

          <h3 className="text-sm font-bold text-stone-400 uppercase tracking-wider mb-8 flex items-center gap-3">
            <div
              className={`w-3 h-3 rounded-full ${stats.netProfit >= 0 ? 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)] animate-pulse' : 'bg-red-500'}`}
            />
          ...
          <div className="space-y-4 relative z-10">
            ...
            <div className="flex justify-between items-center p-4 rounded-2xl bg-stone-950 border border-stone-800/50">
              <div className="flex items-center gap-4">
                <div
                  className="w-12 h-12 rounded-xl bg-red-500/10 flex items-center justify-center text-red-400 text-xl border border-red-500/20"
                  aria-hidden="true"
                >
                  ✂️
                </div>
            ...
            <div className="flex justify-between items-center p-4 rounded-2xl bg-emerald-950/20 border border-emerald-500/20">
              <div className="flex items-center gap-4">
                <div
                  className="w-12 h-12 rounded-xl bg-emerald-500/20 flex items-center justify-center text-emerald-400 text-xl border border-emerald-500/30"
                  aria-hidden="true"
                >
                  💵
                </div>
            ...
            <div className="flex justify-between items-center p-4 rounded-2xl bg-stone-950 border border-stone-800/50">
              <div className="flex items-center gap-4">
                <div
                  className="w-12 h-12 rounded-xl bg-orange-500/10 flex items-center justify-center text-orange-400 text-xl border border-orange-500/20"
                  aria-hidden="true"
                >
                  📦
                </div>
            ...
            <div className="flex justify-between items-center p-4 rounded-2xl bg-stone-950 border border-stone-800/50">
              <div className="flex items-center gap-4">
                <div
                  className="w-12 h-12 rounded-xl bg-rose-500/10 flex items-center justify-center text-rose-400 text-xl border border-rose-500/20"
                  aria-hidden="true"
                >
                  💸
                </div>
            ...
            <div className="pt-6 mt-4 border-t border-stone-800">
              <div className="flex justify-between items-end">
                ...
                <div className="text-right">
                  <p
                    className={`text-5xl font-black tracking-tight ${stats.netProfit >= 0 ? 'text-emerald-400 drop-shadow-[0_0_15px_rgba(52,211,153,0.3)]' : 'text-red-500 drop-shadow-[0_0_15px_rgba(239,68,68,0.3)]'}`}
                  >
                    {stats.netProfit >= 0 ? '+' : ''}
                    {formatCurrency(stats.netProfit)}
                  </p>
                  <p
                    className={`text-sm mt-2 font-bold bg-stone-950 inline-block px-3 py-1 rounded-full border ${stats.netProfit >= 0 ? 'text-emerald-500 border-emerald-500/20' : 'text-red-500 border-red-500/20'}`}
                  >
                    Net Kâr Marjı: %
                    {stats.grossRevenue > 0 ? ((stats.netProfit / stats.grossRevenue) * 100).toFixed(1) : '0.0'}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="lg:col-span-1 grid grid-cols-2 gap-4 h-full">
          <div className="bg-gradient-to-br from-stone-900 to-stone-950 border border-stone-800 rounded-3xl p-6 flex flex-col justify-center relative overflow-hidden group shadow-xl">
            <div
              className="absolute -bottom-6 -right-6 text-7xl opacity-[0.03] group-hover:scale-110 transition-transform"
              aria-hidden="true"
            >
              🍔
            </div>
            <div
              className="w-12 h-12 rounded-2xl bg-amber-500/10 text-amber-500 flex items-center justify-center text-xl mb-4 border border-amber-500/20"
              aria-hidden="true"
            >
              📝
            </div>
          ...
          <div className="bg-gradient-to-br from-stone-900 to-stone-950 border border-stone-800 rounded-3xl p-6 flex flex-col justify-center relative overflow-hidden group shadow-xl">
            ...
            <div
              className="w-12 h-12 rounded-2xl bg-blue-500/10 text-blue-500 flex items-center justify-center text-xl mb-4 border border-blue-500/20"
              aria-hidden="true"
            >
              ⚖️
            </div>
          ...
          <div
            tabIndex={stats.criticalStockCount > 0 ? 0 : -1}
            role={stats.criticalStockCount > 0 ? 'button' : undefined}
            aria-label="Kritik stok detaylarını aç"
            onClick={() => stats.criticalStockCount > 0 && setShowCriticalModal(true)}
            onKeyDown={(e) => e.key === 'Enter' && stats.criticalStockCount > 0 && setShowCriticalModal(true)}
          ...
          >
            ...
            <div
              className={`w-12 h-12 rounded-2xl flex items-center justify-center text-xl mb-4 border ${stats.criticalStockCount > 0 ? 'bg-red-500/20 text-red-500 border-red-500/30 animate-pulse' : 'bg-stone-800 text-stone-500 border-stone-700'}`}
              aria-hidden="true"
            >
              🔔
            </div>
            <p className="text-stone-400 text-xs font-bold uppercase tracking-wider mb-1">Kritik Stok</p>
            <p className={`text-4xl font-black ${stats.criticalStockCount > 0 ? 'text-red-400' : 'text-stone-300'}`}>
          ...
          <div className="bg-gradient-to-br from-stone-900 to-emerald-950/10 border border-stone-800 hover:border-emerald-500/30 transition-all duration-300 rounded-3xl p-6 flex flex-col justify-center relative overflow-hidden group shadow-xl">
            ...
            <div
              className="w-12 h-12 rounded-2xl bg-emerald-500/10 text-emerald-500 flex items-center justify-center text-xl mb-4 border border-emerald-500/20"
              aria-hidden="true"
            >
              💰
            </div>
            <p className="text-stone-400 text-[10px] sm:text-xs font-bold uppercase tracking-wider mb-1 relative z-10">
              Stok Değeri
            </p>
      ...
      <h2 className="text-lg font-bold mb-6 text-stone-300 flex items-center gap-2 uppercase tracking-wider">
        <span aria-hidden="true">🚀</span> Sistem Modülleri
      </h2>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        {modules.map((mod) => (
          <div
            ...
            tabIndex={0}
            role="button"
            aria-label={`${mod.title} modülüne git`}
            onClick={() => router.push(mod.path)}
            onKeyDown={(e) => e.key === 'Enter' && router.push(mod.path)}
          ...
          >
            <div
              className="absolute -right-4 -bottom-4 text-7xl opacity-[0.02] group-hover:opacity-5 transition-opacity group-hover:scale-110"
              aria-hidden="true"
            >
              {mod.icon}
            </div>
            <div
              className="w-14 h-14 rounded-2xl bg-stone-950 border border-stone-800 group-hover:border-amber-500/30 flex items-center justify-center text-3xl mb-4 transition-colors"
              aria-hidden="true"
            >
              {mod.icon}
            </div>
      ...
      {showCriticalModal && (
        <div
          role="dialog"
          aria-modal="true"
          aria-labelledby="critical-stock-modal-title"
          className="fixed inset-0 bg-stone-950/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 transition-opacity"
        >
          <div className="bg-gradient-to-br from-stone-900 to-stone-950 border border-stone-800 rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl relative animate-in fade-in zoom-in duration-200">
            <button
              ...
              aria-label="Kapat"
              className="absolute top-6 right-6 w-8 h-8 flex items-center justify-center rounded-full bg-stone-800 hover:bg-red-500/20 hover:text-red-400 text-stone-400 transition-colors z-10 font-bold"
            >
              ✕
            </button>
            <div className="p-8 pb-6 border-b border-stone-800">
              <div className="flex items-center gap-4">
                <div
                  className="w-12 h-12 rounded-2xl bg-red-500/10 text-red-500 flex items-center justify-center text-2xl border border-red-500/20 animate-pulse"
                  aria-hidden="true"
                >
                  ⚠️
                </div>
                <div>
                  <h2 id="critical-stock-modal-title" className="font-black text-xl text-white">
                    Kritik Stok Uyarıları
                  </h2>
                  <p className="text-stone-400 mt-2 text-sm leading-relaxed">
                    Burası işletmenizin kalbi. Kısayollarla dilediğiniz modüle atlayın veya genel durumu gözden geçirin.
                  </p>
                </div>
              </div>
            </div>
            <div className="p-6 max-h-[50vh] overflow-y-auto space-y-4">
              {stats.criticalItems.map(
                (item: {
                  id: string
                  name: string
                  critical_stock_level: number
                  ...
                  <div
                    key={item.id}
                    className="flex justify-between items-center p-4 rounded-2xl bg-stone-950 border border-stone-800 hover:border-red-500/30 transition-colors"
                  >
                    <div>
                      ...
                      <p className="text-stone-400 text-xs mt-1 font-bold uppercase tracking-wider">
                        ...
                        <span className="text-stone-300">
                          {item.critical_stock_level} {item.unit}
                        </span>
                      </p>
                    </div>
                    <div className="text-right">
                      ...
                      <p
                        className={`font-black text-2xl ${item.stock_quantity <= 0 ? 'text-red-500' : 'text-amber-500'}`}
                      >
              ...
              {stats.criticalItems.length === 0 && (
                <div className="text-center p-8 text-stone-500 font-medium">Şu an kritik seviyede stok bulunmuyor.</div>
              )}
            </div>
