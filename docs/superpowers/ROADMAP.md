...
| `SEC-02`     | **Kalan `SECURITY DEFINER` bulguları**<br>Sonuç: every remaining advisor finding will receive an evidence-backed classification.                  | Hazır        | Sonraki: create a focused security review plan before any fix.<br>Detay: [Özel finansal belge rollout](../security/private-financial-document-rollout.md)<br>Kanıt: current production advisor snapshot records 13 callable-surface warnings; refresh at task start. |
