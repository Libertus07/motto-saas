...
Mevcut `user_id` alanları semantik olarak `created_by` veya `performed_by` kabul edilmelidir. Yeni güvenlik tasarımında aktör değeri istemciden güvenilir kabul edilmemeli, mümkün olan her yerde `(select auth.uid())` ile atanmalıdır (caching performansı için).
...
Aşağıdaki tablolarda `organization_id uuid not null` doğrudan bulunmalıdır. İlişki tablolarında tenant ebeveynden türetilebilse bile doğrudan kolon kullanılması RLS sadeliği, sorgu performansı ve savunma katmanı nedeniyle tercih edilmiştir. **ÖNEMLİ:** RLS sorgularının performanslı çalışabilmesi için, eklenen her `organization_id` kolonu (`CREATE INDEX` ile) mutlaka indekslenmelidir.
...
*(Not: Performans için oluşturulan her bileşik (composite) Foreign Key'e karşılık gelen bir `CREATE INDEX` komutu da mutlaka eklenmelidir.)*
...
```sql
using (
    -- NOT: Performans için auth.uid() doğrudan kullanılmaz, (select auth.uid()) ile önbelleklenir.
    -- Daha iyi bir pratik olarak private schema içinde SECURITY DEFINER bir fonksiyon kullanılmalıdır.
    organization_id IN (SELECT private.get_user_organizations())
)
```
...
1. Aktörü `(select auth.uid())` ile belirlemeli (caching performansı için).
...
| RPC'ye başka kullanıcıya ait `user_id` gönderilir | Değer yok sayılır; aktör `(select auth.uid())` olur |
